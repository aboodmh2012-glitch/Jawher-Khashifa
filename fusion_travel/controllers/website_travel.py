...
class FusionTravelWebsiteController(http.Controller):
    @http.route(['/travel', '/travel/'], type='http', auth='public', website=True, sitemap=True)
    def travel_home(self, **kwargs):
        return request.render('fusion_travel.flights_search_template', {})

    @http.route('/travel/flights', type='http', auth='public', website=True)
    ...
    def _get_offer_by_token(self, token, service_type):
        token = (token or "").strip()
        if not token:
            raise ValidationError(_("Select a valid provider offer."))
    ...
    @http.route('/travel/flights/search', type='http', auth='public', website=True, methods=['POST'], csrf=True)
    def flights_search(self, **post):
        ...
        try:
            ...
            session = self._create_search_session('flight', vals)
            response = AmadeusFlights(request.env).search_offers(vals)
            raw_offers = response.get('data', []) if isinstance(response, dict) else []
            for raw_offer in raw_offers[:20]:
                if isinstance(raw_offer, dict):
                    offer = dict(raw_offer)
                    price = offer.get('price') or {}
                    stored = self._store_offer(
                        session, offer, self._safe_float(price.get('grandTotal') or price.get('total')), price.get('currency')
                    )
                    offer['_token'] = stored.token
                    offers.append(offer)
        ...
        return request.render('fusion_travel.trip_search_results_template', {
            'search': vals,
            'offers': offers,
    ...
    @http.route('/travel/flights/book', type='http', auth='user', website=True, methods=['POST'], csrf=True)
    def flights_book_offer(self, **post):
        try:
            with request.env.cr.savepoint():
                stored_offer, offer = self._get_offer_by_token(post.get('offer_token'), 'flight')
                price = offer.get('price') or {}
                ...
                booking = request.env['fusion.travel.booking'].sudo().create({
                    'partner_id': request.env.user.partner_id.id,
    ...
    @http.route('/travel/booking/<int:booking_id>/passengers', type='http', auth='user', website=True, methods=['GET'], csrf=True)
    def booking_passengers_page(self, booking_id, **kwargs):
        try:
            ...
            return request.render('fusion_travel.trip_passenger_details_template', {
                'booking': booking,
    ...
    def booking_passengers_submit(self, booking_id, **post):
        try:
            ...
            booking.sudo().write({'passenger_ids': commands})
            return request.redirect(f'/travel/booking/{booking.id}/confirm-price')
        except (UserError, ValidationError) as exc:
            return request.render('fusion_travel.trip_passenger_details_template', {
                'booking': self._get_user_booking(booking_id),
    ...
    @http.route('/travel/booking/<int:booking_id>/confirm-price', type='http', auth='user', website=True, methods=['GET', 'POST'], csrf=True)
    def booking_confirm_price(self, booking_id, **post):
        try:
            ...
            return request.render('fusion_travel.trip_price_confirmation_template', {
                'booking': self._get_user_booking(booking_id),
                'error': str(exc),
            })
        except Exception:  # noqa: BLE001
    ...
    @http.route('/travel/booking/<int:booking_id>/payment', type='http', auth='user', website=True, methods=['GET', 'POST'], csrf=True)
    def booking_payment(self, booking_id, **post):
        try:
            ...
            return request.render('fusion_travel.trip_payment_template', {'booking': self._get_user_booking(booking_id), 'error': str(exc)})
        except Exception:  # noqa: BLE001
            _logger.exception('Payment step failed')
            return request.render('fusion_travel.trip_payment_template', {
                'booking': self._get_user_booking(booking_id),
                'error': _('Payment could not be completed. Please contact support.'),
            })

    @http.route('/travel/booking/<int:booking_id>/issue', type='http', auth='user', website=True, methods=['GET', 'POST'], csrf=True)
    def booking_issue(self, booking_id, **post):
        try:
            ...
            return request.render('fusion_travel.trip_issue_template', {'booking': self._get_user_booking(booking_id), 'error': str(exc)})
        except Exception:  # noqa: BLE001
            _logger.exception('Supplier order creation failed')
            return request.render('fusion_travel.trip_issue_template', {
                'booking': self._get_user_booking(booking_id),
    ...
    # Hotels
    # ------------------------------------------------------------------
    @http.route('/travel/hotels', type='http', auth='public', website=True)
    def hotels_page(self, **kwargs):
        return request.render('fusion_travel.hotels_search_template', {})
    ...
    @http.route('/travel/hotels/search', type='http', auth='public', website=True, methods=['POST'], csrf=True)
    def hotels_search(self, **post):
        ...
        try:
            ...
            session = self._create_search_session('hotel', vals)
            ...
            for item in raw[:12]:
                ...
                for offer in (item.get('offers') or [])[:2]:
                    if not isinstance(offer, dict):
                        continue
                    payload = {'hotel': hotel_info, 'offer': offer}
                    price = offer.get('price') or {}
                    stored = self._store_offer(session, payload, self._safe_float(price.get('total')), price.get('currency'))
                    hotels.append({'hotel': hotel_info, 'offer': offer, '_token': stored.token})
        ...
        return request.render('fusion_travel.trip_hotel_results_template', {
            'search': vals,
            'hotels': hotels,
            'error': error,
        })

    @http.route('/travel/hotels/book', type='http', auth='user', website=True, methods=['POST'], csrf=True)
    def hotels_book_offer(self, **post):
        try:
            with request.env.cr.savepoint():
                stored_offer, payload = self._get_offer_by_token(post.get('offer_token'), 'hotel')
    ...
    # Cars / Transfers
    # ------------------------------------------------------------------
    @http.route('/travel/cars', type='http', auth='public', website=True)
    def cars_page(self, **kwargs):
        return request.render('fusion_travel.cars_search_template', {})
    ...
    @http.route('/travel/cars/search', type='http', auth='public', website=True, methods=['POST'], csrf=True)
    def cars_search(self, **post):
        ...
        try:
            ...
            session = self._create_search_session('car', vals)
            response = AmadeusCars(request.env).search_transfers(vals)
            raw = response.get('data', []) if isinstance(response, dict) else []
            for raw_offer in raw[:20]:
                if isinstance(raw_offer, dict):
                    offer = dict(raw_offer)
                    quotation = offer.get('quotation') or {}
                    stored = self._store_offer(session, offer, self._safe_float(quotation.get('monetaryAmount')), quotation.get('currencyCode'))
                    offer['_token'] = stored.token
                    offers.append(offer)
        except (UserError, ValidationError, ValueError) as exc:
            error = str(exc)
        ...
        return request.render('fusion_travel.trip_car_results_template', {
            'search': vals,
            'offers': offers,
            'error': error,
        })

    @http.route('/travel/cars/book', type='http', auth='user', website=True, methods=['POST'], csrf=True)
    def cars_book_offer(self, **post):
        try:
            with request.env.cr.savepoint():
                stored_offer, offer = self._get_offer_by_token(post.get('offer_token'), 'car')
                quotation = offer.get('quotation') or {}
